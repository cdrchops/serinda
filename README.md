...
- Pytorch
- Flask
- Expanding SERINDA the way I had it with Flask
- Rust where possible

### Hardware  
 - OAK-D  
	- has IMU - should be easy to integrate what I need  
	- [depthai-python/docs/source/components/nodes/imu.rst at main · luxonis/depthai-python · GitHub](https://github.com/luxonis/depthai-python/blob/main/docs/source/components/nodes/imu.rst)  
	- [DepthAI’s Documentation — DepthAI documentation | Luxonis](https://docs.luxonis.com/en/latest/)  
	- need a smaller version for an HMD later  
	- What about SLAM/VSLAM/ORB SLAM  
	- can I hotswap different models in camera?  
- Headset  
- screen  
- eye portion  
- lens (magnifier)  
...
- Audio Input  
	- SpeechRecognition  
	- May run audio through google or other sources  
	- would really like to have this local though  
- Audio Output  
...
- keyboard commands  
- audio output  
- IMU tracking for location  
- depth tracking for adding overlays  
- BabylonJS for AR Views?  
	- billboards, virtual screens, etc  
	- xyz coords need to be sent from IMU to babylonjs  
	- some way to map that  
	- how did triton map that to threejs - even though it was just hand detection  
	- pin objects in space  
	- pin objects to view  
	- Look at SimpleAR  
	- Look at Zap.ar  
- Hand tracking - [GitHub - geaxgx/depthai\_hand\_tracker: Running Google Mediapipe Hand Tracking models on Luxonis DepthAI hardware (OAK-D-lite, OAK-D, OAK-1,...)](https://github.com/geaxgx/depthai_hand_tracker)  
- OpenCV tracking  
	- Quickly map a "target"  
    - Then track that target - like target lock  
       - anytime it comes into view it's identified correctly  
- OpenCV via WebRTC  
- OAK examples - can I use the OpenCV/OAK examples to integrate a 3rd camera with a zoom that could be calibrated with the OAK cameras to set it to locations in 3d space?  
- OpenVINO - optimizing and deploying AI inference  
- Is Unity better than BabylonJS -> WebGL 
	- Can it do the things I want it to do within the parameters I've defined?  
    - Do I need to use Unity or can I use BabylonJS  
    - Is Unity easier to use?  
    - Can I hook IMU data into Unity to do what I want?  
- OAK to webpage  
	- display  
	- via flask  
    - BJS hooked up with IMU  
	    - move items out of view and look around  
- HMD set up to show display  
- Air keyboard  
- ID object and put billboard over that object  
	- mark as temp placement OR  
	- mark when to show -> turn on specific object or type (like chairs)  
	- permanently pin in 3D space  
- VIO/SLAM  
	- mapping -> how much expense is involved in the data storage of SLAM mapping  
	- workspaces that I can move around  
		- like having multiple monitors  
		- can pin in front  
		- set in 3D space  
		- or "minimize" until needed  
    - VIO to see an object and map it to be able to work on it like a car engine or whatever...   
		- like maybe LEGO assembly (detect which piece is needed and how it should go togehter)  
    - Use handgestures to select items  
		- for tracking  
...
- https://www.jovo.tech/  
- Also go back through the show Continuum and organize my screenshots to see what all they have as ideas and options then map the ones I like out  
  
### Mixed Reality  
- What is this application for
- [The Important Difference Between Augmented Reality And Mixed Reality | Bernard Marr](https://bernardmarr.com/the-important-difference-between-augmented-reality-and-mixed-reality/)  
  
### Questions  
- [ ] How does unity work in this field?  
   - [ ] Is it easy?  
   - [ ] How well does it integrate for MR?  
- [ ] BabylonJS for MR?  
   - [ ] does it lend itself well?  
   - [ ] are there any examples from anyone else?  
   - [ ] I still have my work I did for multiple cameras   

### NOTES  
- Tesla uses OpenCV and Pytorch  
- OpenCV uses Pytorch  
- Pytorch for ML  
- Tesla also uses YOLO to some extent
