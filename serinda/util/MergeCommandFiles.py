...
class MergeCommandFiles:
    ...
    def mergeFiles(self):
        self.fileMerger("commands.yml", ApplicationConstants.serindaCommandsYmlFile)
        self.fileMerger("commands.jsgf", ApplicationConstants.serindaCommandsJSGFFile)


    def fileMerger(self, directoryFiles, outputFile):
        pathlist = Path(ApplicationConstants.pluginPath).glob('**/' + directoryFiles)
